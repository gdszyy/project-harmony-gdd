...
func _on_player_died() -> void:
	# TODO: 迁移到 EventBus.subscribe(Events.PLAYER_DIED, ...) 模式
	pass
...
func _connect_upgrade_signals() -> void:
	...
	# 【注意】不再桥接到 GameManager.upgrade_selected，避免双重触发
...
func _on_upgrade_chosen_v3(upgrade: Dictionary) -> void:
	## 【修复】此回调不再桥接到 GameManager.upgrade_selected。
...
func _on_inscription_acquired(inscription: Dictionary) -> void:
	# TODO: 迁移到 EventBus.subscribe(Events.INSCRIPTION_ACQUIRED, ...) 模式
	pass

func _on_easter_egg_triggered(egg: Dictionary) -> void:
	# TODO: 迁移到 EventBus.subscribe(Events.EASTER_EGG_TRIGGERED, ...) 模式
	pass
