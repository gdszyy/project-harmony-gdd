...
v3.0 将原先分散的多个面板整合为统一的 **一体化编曲台 (Integrated Composer)** 界面 (`integrated_composer.gd`)，在一个视图内完成了音符库存管理、序列器编排、和弦炼成和手动槽配置的全部流程，极大地提升了操作的流畅性和直观性。
...
v3.0 对局内升级系统进行了彻底重构，将 **五度圈罗盘 (`circle_of_fifths_upgrade_v3.gd`)** 确立为唯一的局内升级入口，废除了传统的“三选一”卡片界面。

新的升级流程分为两个阶段：

1.  **方向选择**：罗盘外围浮现三个方向性选项——**进攻 (顺时针)**、**核心 (当前)**、**防御 (逆时针)**。
2.  **具体选择**：玩家选定一个方向后，系统会从该方向的专属升级池中随机抽取2-3个具体升级项供玩家最终选择。

此外，新系统还引入了 **乐理突破 (Theory Breakthrough)** 事件，作为一种稀有的特殊升级，可直接解锁核心游戏机制。局外成长系统解锁的升级也会在选项中以特殊金色标识高亮显示，增强了局内外成长的联动性。

| 方向 | 调性走向 | 升级倾向 | 示例 |
| :--- | :--- | :--- | :--- |
| 方向 | 调性走向 | 升级倾向 | 示例 |
| :--- | :--- | :--- | :--- |
| **进攻 (顺时针)** | 更“明亮”的调 | 伤害、速度、进攻性词条 | “音波增幅”、“节奏加速”、“穿透精通” |
| **防御 (逆时针)** | 更“柔和”的调 | 生存、疲劳抗性、资源获取 | “生命强化”、“单调耐受”、“电子乐变体” |
| **核心 (当前)** | 停留在当前音级 | 核心机制强化、音符获取 | “全维强化”、“音符丰收”、“复合修饰” |



v3.0 的五度圈罗盘系统由 `circle_of_fifths_upgrade_v3.gd` 实现，其核心逻辑包括：两阶段选择流程、方向性升级池、乐理突破事件触发、以及与局外成长系统的可视化联动。
...
本次更新是对游戏核心交互流程的全面重构，旨在解决v2版本中界面分散、升级随机性过高的问题，提升玩家的策略深度和操作流畅度。

**核心改动：**
- **P0 - 一体化编曲台 (`integrated_composer.gd`)**：将原先分散的和弦炼成、法术书、序列器和手动槽配置四个面板整合为一个统一的“编曲台”界面，实现了在一个视图内完成所有法术编辑工作，支持拖拽式和弦合成与法术配置。
- **P1 - 统一升级系统 (`circle_of_fifths_upgrade_v3.gd`)**：废除三选一卡片，将五度圈罗盘升级为唯一的局内升级入口。采用“方向选择 → 具体升级”的两阶段模式，增强了玩家选择的策略性。
- **P2 - 反馈增强 (`meta_progression_visualizer.gd`)**：新增局外成长技能树可视化界面，并在局内升级选项中高亮显示局外解锁的升级。引入“乐理突破”事件，作为解锁核心机制的稀有奖励。
- **P3 - 数值与权重调整 (`balance_config_v3.gd`)**：将所有核心数值（音符属性、和弦倍率、疲劳参数、升级权重等）集中到统一的配置文件中，并对白键音符的四维参数进行了极化调整，增强了不同音符的独特性。

**相关文档：**
- [评估报告与v3.0升级方案](evaluation_and_proposal.md)
...
### 8.2. 局外成长系统：和谐殿堂 (Meta-Progression)
