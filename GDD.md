...
敌人的设计同样遵循“不和谐的具象化”原则，每一种敌人都代表一种特定的“音乐错误”或“音频故障”。

| 敌人类型 | 音乐隐喻 | 行为模式 | 音效特点 |
|---|---|---|---|
| **Static (静电)** | 随机噪声 | 基础的追踪敌人，移动模式带有随机性。 | 短促的数字噪声点击 (`noise_click`) |
| **Pulse (脉冲)** | 错误的节拍器 | 按固定节拍进行冲刺或发射弹幕，打乱玩家节奏。 | 脉冲滴答声 (`pulse_tick`) |
| **Screech (尖啸)** | 麦克风反馈音 | 快速接近，死亡时爆发出刺耳的反馈音波。 | 反馈尖叫声 (`feedback_whine`) |
| **Silence (寂静)** | 休止符 / 黑洞 | 吞噬声音，靠近玩家时增加“疲劳度”。 | 低沉的嗡嗡声 (`low_hum`) |
| **Wall (音墙)** | 砖墙限制器 | 巨大的阻挡者，缓慢推进，迫使玩家走位。 | 沉重的研磨声 (`heavy_grind`) |

详细的敌人系统设计请参阅 **[敌人系统设计文档](Docs/Enemy_System_Design.md)**。
