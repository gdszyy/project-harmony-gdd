...
为了将空间音频从简单的沉浸感增强工具转变为功能性的非视觉信息通道，系统已实现以下扩展：

### 4.1. 距离子总线架构

在 `EnemySFX` 总线下新增三个距离子总线，各自挂载不同截止频率的低通滤波器：

| 子总线 | LPF 截止频率 | 距离范围 | 听感 |
|---|---|---|---|
| `EnemySFX_Near` | 无 LPF | 0-200px | 清晰明亮 |
| `EnemySFX_Mid` | 6000 Hz | 200-500px | 略显沉闷 |
| `EnemySFX_Far` | 2500 Hz | 500px+ | 模糊遥远 |

### 4.2. 状态音效映射

敌人进入特定状态时，`SpatialAudioController` 会自动叠加对应的音频效果参数：

| 状态 | 效果 | 触发时机 |
|---|---|---|
| 眩晕 (Stunned) | 音高振荡模拟镜边 | `apply_stun()` |
| 精英 (Elite) | 音量增大 + 音高降低 | 精英初始化 |
| 蓄力 (Charging) | LPF 扫频上升 | 精英攻击执行 |
| 低血量 (Low Health) | 音量颤音 | HP < 30% |
| 护盾 (Shielded) | 轻微移相 | 护盾激活 |

详细设计请参见 **[OPT06 空间音频信息传递设计文档](../Optimization_Modules/OPT06_SpatialAudioInformationDelivery.md)**。
...
| **敌人音效** | 白噪音、Bitcrush、电流声 | 基于信号触发，由 `AudioManager` 播放程序化生成的噪音，与 BGM 不合拍。**已集成 OPT06 空间音频信息传递。** | 制造冲突感，提供清晰的受击、死亡等负反馈，同时通过空间化传递方位和状态信息。 |
| **音频总线** | Master, Music, SFX (Enemy, Player, UI) | `Music` 总线挂载频谱分析器，`EnemySFX` 下新增 Near/Mid/Far 距离子总线并挂载 LPF。 | 隔离音乐与音效，便于混音与效果处理，支持基于距离的音色调制。 |
| **实现架构** | 全局单例 (AudioManager, BGMManager) | 对象池、程序化音效生成、信号驱动、**空间音频控制器 (OPT06)**。 | 高性能、低耦合、易于扩展和维护。 |
