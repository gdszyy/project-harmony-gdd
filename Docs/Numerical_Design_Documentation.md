...
每个音符的有效射程由其速度和持续时间决定：有效射程 = SPD × 200px/s × DUR × 0.5s。基准射程设定为900px（C音符的射程），低于此值的音符会受到命中率折扣，但大弹体（高SIZE）可以部分补偿。

| 音符 | 有效射程(px) | 基础距离因子 | SIZE补偿 | 调整后因子 | 设计说明 |
| :---: | :---: | :---: | :---: | :---: | :--- |
| **C** | 900 | 1.000 | 0.10 | 1.000 | 基准音符，无折扣 |
| **D** | 1500 | 1.000 | 0.00 | 1.000 | 远程狙击型，射程最远 |
| **E** | 800 | 0.889 | 0.20 | 0.911 | SIZE补偿后几乎无折扣 |
| **F** | 500 | 0.556 | 0.20 | 0.644 | 近战型，高SIZE补偿但仍有显著折扣 |
| **G** | 600 | 0.667 | 0.00 | 0.667 | 爆发型，无SIZE补偿，高DMG作为补偿 |
| **A** | 800 | 0.889 | 0.00 | 0.889 | 轻微折扣 |
| **B** | 800 | 0.889 | 0.00 | 0.889 | 轻微折扣 |
...
本次 v2.1 数值设计在 v2.0 的基础上，进一步引入了**延迟与距离风险维度**，使跑分模型能够更全面地评估法术的实战表现。通过将延迟命中折扣和距离命中折扣纳入有效伤害计算，并将延迟空窗和近身风险纳入风险评分，系统现在能够验证“高延迟/短射程法术用伤害/范围补偿”这一设计原则是否得到了合理的落实。
