...
## 9. 章节音色武器系统数值平衡 (v2.3 — Issue #38)

本节定义了 v2.0 音色武器系统重新设计后的所有数值参数及其平衡性约束。音色系统从原来的四大声学系别（弹拨/拉弦/吹奏/打击）重新设计为七个章节专属音色武器，并引入了章节词条系统和电子乐变体。

### 9.1. 音色武器核心机制数值

每种章节音色武器拥有独特的核心机制，其数值参数如下表所示。所有数值均为基础值，可被局内升级和词条协同效果修改。

| 音色武器 | 核心机制 | 关键参数 | 基础值 | 可升级上限 |
| :--- | :--- | :--- | :---: | :---: |
| **里拉琴** | 泛音共鸣 | 共鸣伤害比例 | 15% | 30% |
| | | 共鸣半径 | 60px | 100px |
| | | 2:1 比例加成 | +30% | +50% |
| | | 3:2 比例加成 | +20% | +35% |
| | | 4:3 比例加成 | +10% | +20% |
| **管风琴** | 和声层叠 | 最大声部层 | 4 层 | 6 层 |
| | | 每层碰撞范围加成 | +10% | +15% |
| | | 每层伤害加成 | +8% | +12% |
| | | 层消退时间 | 3.0s | 5.0s |
| | | 圣咏区域持续 | 2.0s | 4.0s |
| **羽管键琴** | 对位交织 | 对位弹体延迟 | 0.2s | 0.1s |
| | | 对位弹体伤害比 | 50% | 80% |
| | | 对位共鸣额外伤害 | +30% | +50% |
| **钢琴** | 力度动态 | forte 伤害倍率 | x1.5 | x2.0 |
| | | mezzo 伤害倍率 | x1.0 | x1.2 |
| | | piano 伤害倍率 | x0.7 | x0.8 |
| | | forte 判定窗口 | ±50ms | ±80ms |
| **管弦全奏** | 情感爆发 | 情感增量/攻击 | +2 | +4 |
| | | 情感增量/受伤 | +15 | +25 |
| | | 情感衰减/秒 | -3 | -1.5 |
| | | fortissimo 伤害倍率 | x1.5 | x2.0 |
| | | 高潮爆发伤害倍率 | x2.0 | x3.0 |
| | | 高潮爆发持续 | 3.0s | 5.0s |
| **萨克斯** | 摇摆攻击 | 反拍伤害加成 | +25% | +50% |
| | | 即兴独奏触发次数 | 3 次 | 2 次 |
| | | 即兴独奏持续 | 5.0s | 8.0s |
| | | 即兴独奏施法速度 | +30% | +50% |
| **合成主脑** | 波形变换 | 对应属性加成 | +30% | +50% |
| | | 其他属性惩罚 | -10% | -5% |
| | | 变形过渡时间 | 0.5s | 0.2s |
...
| 参数 | 值 | 设计说明 |
| :--- | :---: | :--- |
| 当前章节专属音色疲劳 | 0.00/次 | 使用当前章节专属音色武器不产生额外疲劳 |
| 跨章节音色疲劳 | 0.03/次 | 低于 v1.0 的 0.05/次，允许跨章节使用但有代价 |
| 电子乐变体疲劳减免 | -50% | 电子乐与游戏风格契合，获得疲劳优待 |
| 中度疲劳效能衰减 | -20% | AFI > 0.5 时音色核心机制效能降低 |
| 严重疲劳效能衰减 | -50% | AFI > 0.8 时效能大幅降低，词条协同失效 |

### 9.3. 章节词条数值

每个章节有 3 个专属词条（普通/稀有/史诗各 1 个），共 21 个词条。词条在升级面板中以 **15%** 的概率替代普通升级选项出现。

**词条数值平衡约束**：

| 稀有度 | 伤害加成范围 | 特殊效果强度 | 协同加成幅度 |
| :--- | :---: | :---: | :---: |
| 普通 | +10%~+25% | 低（被动触发） | 便利性提升 |
| 稀有 | +15%~+56% | 中（条件触发） | 效果增强 30%~100% |
| 史诗 | +50%~+200% | 高（大型触发） | 解锁隐藏效果 |

**跨章节词条组合（音乐史彩蛋）**：

| 组合名 | 触发条件 | 效果强度 |
| :--- | :--- | :--- |
| 古今交响 | Ch1 + Ch5 史诗词条 | 两个终极效果可同时触发 |
| 对位与即兴 | Ch3 + Ch6 稀有词条 | 传导伤害可触发即兴独奏 |
| 数字毕达哥拉斯 | Ch1 + Ch7 任意词条 | 距离判定精度放宽 20% |
| 从记谱到故障 | Ch2 + Ch7 词条 | 谱线变为故障数据流，伤害 +15% |
| 命运与形式 | Ch4 + Ch5 词条 | 阶段切换触发命运动机第 4 击 |

### 9.4. 音色武器 DPS 估算

以下为各音色武器在标准条件下（BPM=120，无升级，无词条）的预估 DPS 贡献。音色武器的核心机制作为弹体行为修饰器，其 DPS 贡献以基础弹体 DPS 的百分比表示。

| 音色武器 | 核心机制 DPS 贡献 | 和弦交互 DPS 贡献 | 综合 DPS 修正 |
| :--- | :---: | :---: | :---: |
| 里拉琴 | +15%~+30% (距离依赖) | +10%~+20% (区域型) | +25%~+50% |
| 管风琴 | +8%~+32% (层数依赖) | +8%~+32% (DOT递增) | +16%~+64% |
| 羽管键琴 | +50% (对位弹体) +30% (共鸣) | +50% (双爆炸) | +80%~+130% |
| 钢琴 | -30%~+50% (力度依赖) | +20% (召唤延长) | -10%~+70% |
| 管弦全奏 | -20%~+100% (情感依赖) | +50% (冲击波增强) | -20%~+150% |
| 萨克斯 | +25% (反拍) +30% (即兴) | +20% (反击) | +25%~+75% |
| 合成主脑 | +30% (波形加成) -10% (惩罚) | +故障效果 (控制) | +20%~+30% |

> **平衡性说明**：高上限的音色武器（如管弦全奏 +150%）需要满足严格的触发条件（情感强度 100 + fortissimo 状态），而低门槛的音色武器（如合成主脑 +20%~+30%）提供稳定但较低的增益。这确保了"高风险高回报"的设计原则在音色武器层面也得到体现。

### 9.5. 跑分系统扩展

为验证音色武器系统的平衡性，`balance_scorer.py` 需要扩展以下模拟场景：

1. **章节专属 Build**：模拟玩家在每个章节中使用专属音色武器 + 全部 3 个章节词条的最优 Build。
2. **跨章节 Build**：模拟玩家使用非当前章节音色武器的 Build，验证跨章节疲劳惩罚是否有效。
3. **电子乐变体 Build**：模拟使用电子乐变体的 Build，验证疲劳减免是否合理。
4. **音乐史彩蛋 Build**：模拟触发跨章节词条组合的 Build，验证彩蛋效果是否过强。

**预期跑分结果**：

| 场景 | 预期综合得分 | 预期风险评分 |
| :--- | :---: | :---: |
| 章节专属 Build（最优） | 85~95 | 30~50 |
| 跨章节 Build | 60~75 | 50~70 |
| 电子乐变体 Build | 80~90 | 25~40 |
| 音乐史彩蛋 Build | 90~100 | 40~60 |
| 无音色武器基线 | 50~60 | 20~30 |
