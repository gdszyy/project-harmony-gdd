...
AFI 是五个核心疲劳维度的加权总和。每个维度的“疲劳值”都被归一化到 `[0.0, 1.0]` 区间，其中 `0.0` 代表最高多样性（无疲劳），`1.0` 代表最低多样性（极度疲劳）。
...
为了帮助玩家理解疲劳机制并主动摆脱单调，系统会根据当前疲劳的主要来源，动态生成可操作的恢复建议，例如：
...
-   **场景1 (单调模式):** 反复使用同一音符`C`，AFI在几次施法后迅速飙升至`CRITICAL`等级 (0.9+)，伤害乘数降至几乎为零。这证明了系统对重复操作的敏感性。
-   **场景2 (多样化模式):** 交替使用多种音符与和弦，AFI始终保持在`MILD`或`NONE`的低水平，系统几乎不产生惩罚。这表明模型能正确识别并奖励多样性。
-   **场景3 (恢复过程):** 先进行单调操作使AFI升高，然后切换为多样化操作。结果显示AFI随之平滑下降，从`CRITICAL`逐步恢复到`MODERATE`，验证了系统的恢复机制。
-   **场景5 (疲劳热力图):** 展示了`get_note_fatigue_map`的用途，可以清晰地看到被过度使用的音符`C`和`E`积累了显著高于其他音符的个体疲劳值。
