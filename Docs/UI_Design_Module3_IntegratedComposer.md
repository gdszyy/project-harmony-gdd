...
*   **主题与样式:** 将所有在《美术与VFX方向总文档》[2] 中定义的颜色、字体和样式配置到一个全局的 `Theme` 资源 (`.tres`) 中，并将其应用到编曲台场景的根节点。这可以全局统一所有子 `Control` 节点的外观，方便未来进行统一的风格迭代。辉光、噪点和边框等复杂视觉效果，可以通过为 `PanelContainer` 或 `NinePatchRect` 节点附加自定义的 `ShaderMaterial` 来高效实现。
