## sequencer_ui.gd
## 序列器 UI（增强版 v3.0 — 交互优化）
## 4小节×4拍的乐谱序列器界面
##
## v3.0 交互优化：
##   - 增强模式切换按钮：图标+文字+快捷键提示+动画反馈
##   - 预设模板系统：一键填充推荐编排（6种节奏型模板）
##   - 实时节奏型预览：编辑时即时显示节奏型效果说明
##   - 小节级批量操作：复制/清空/填充小节的右键菜单
##   - 增强音符信息面板：四维属性条形图 + 音符特点描述
##   - 统一使用全局 UI 色彩规范 (UIColors)
##   - 拖拽交互增强：拖拽时显示目标位置预览
##   - 快捷键覆盖层：按住 H 显示所有快捷键
extends Control
...
const PALETTE_Y_OFFSET := 70.0

## 信息面板配置
const INFO_PANEL_Y_OFFSET := 115.0
const INFO_PANEL_HEIGHT := 65.0
const STAT_BAR_WIDTH := 40.0
const STAT_BAR_HEIGHT := 8.0

## 预设模板面板
const TEMPLATE_Y_OFFSET := 185.0

## 颜色定义 — 使用全局调色板
const BG_COLOR := Color(0.04, 0.03, 0.08, 0.9)
...
func _draw() -> void:
	...
	draw_string(font, Vector2(start_x, 15), "SEQUENCER", HORIZONTAL_ALIGNMENT_LEFT, -1, 11, Color(0.6, 0.55, 0.75))
	...
	draw_string(font, Vector2(start_x + 90, 15), mode_text, HORIZONTAL_ALIGNMENT_LEFT, -1, 10, mode_color)
	...
	for measure in range(MEASURES):
		...
		for beat in range(BEATS_PER_MEASURE):
			...
			if idx < _sequencer_data.size():
				...
				if slot.get("type", "") == "note":
					...
					draw_string(font, rest_pos, "-", HORIZONTAL_ALIGNMENT_CENTER, -1, 16, Color(0.4, 0.35, 0.5, 0.5))
		...
		if measure < _measure_rhythms.size() and not _measure_rhythms[measure].is_empty():
			...
			var rhythm_desc := _measure_rhythm_descs[measure] if measure < _measure_rhythm_descs.size() else ""
			# 节奏型名称（高亮）
			...
			# 效果描述（次色）
			if not rhythm_desc.is_empty():
				draw_string(font, Vector2(rhythm_x - 30, start_y + CELL_SIZE.y + 24), rhythm_desc, HORIZONTAL_ALIGNMENT_CENTER, -1, 7, Color(0.5, 0.45, 0.6, 0.6))
	...
	# ========== 音符信息面板 ==========
	_draw_note_info_panel(start_x, start_y + INFO_PANEL_Y_OFFSET, font)

	# ========== 预设模板面板 ==========
	_draw_template_panel(start_x, start_y + TEMPLATE_Y_OFFSET, font)
	...
	draw_string(font, Vector2(start_x + total_width - 70, 15), beat_text, HORIZONTAL_ALIGNMENT_RIGHT, -1, 9, Color(0.4, 0.35, 0.5))
...
func _draw_note_palette(start_x: float, start_y: float, font: Font) -> void:
	...
	for j in range(modes.size()):
		...
		var is_active := _edit_mode == modes[j]["mode"]
...
func _draw_note_info_panel(start_x: float, start_y: float, font: Font) -> void:
	var panel_width := custom_minimum_size.x - 20
	draw_rect(Rect2(Vector2(start_x, start_y), Vector2(panel_width, INFO_PANEL_HEIGHT)), Color(0.05, 0.04, 0.1, 0.6))

	var stats: Dictionary = MusicData.WHITE_KEY_STATS.get(_selected_note, {})
	if stats.is_empty():
		return

	var note_name: String = stats.get("name", "?")
	var note_desc: String = stats.get("desc", "")
	var note_color: Color = MusicData.NOTE_COLORS.get(_selected_note, Color.WHITE)

	# 音符名称和描述
	draw_string(font, Vector2(start_x + 8, start_y + 14), "%s — %s" % [note_name, note_desc], HORIZONTAL_ALIGNMENT_LEFT, -1, 11, note_color)

	# 四维属性条形图
	var stat_names := ["DMG", "SPD", "DUR", "SIZE"]
	var stat_keys := ["dmg", "spd", "dur", "size"]
	var stat_colors := [
		Color(1.0, 0.3, 0.3),   # DMG - 红
		Color(0.3, 0.6, 1.0),   # SPD - 蓝
		Color(0.0, 0.8, 0.4),   # DUR - 绿
		Color(1.0, 0.8, 0.0),   # SIZE - 金
	]

	var bar_start_x := start_x + 8
	var bar_start_y := start_y + 22

	for i in range(4):
		var stat_val: float = stats.get(stat_keys[i], 0)
		var max_val := 5.0
		var fill_ratio := clamp(stat_val / max_val, 0.0, 1.0)

		var label_x := bar_start_x + i * (STAT_BAR_WIDTH + 50)
		var bar_x := label_x + 30

		# 属性名
		draw_string(font, Vector2(label_x, bar_start_y + 8), stat_names[i], HORIZONTAL_ALIGNMENT_LEFT, -1, 8, Color(0.5, 0.45, 0.6))
		# 数值
		draw_string(font, Vector2(label_x, bar_start_y + 20), "%d" % int(stat_val), HORIZONTAL_ALIGNMENT_LEFT, -1, 10, stat_colors[i])

		# 条形图背景
		draw_rect(Rect2(Vector2(bar_x, bar_start_y + 2), Vector2(STAT_BAR_WIDTH, STAT_BAR_HEIGHT)), Color(0.1, 0.08, 0.15, 0.5))
		# 条形图填充
		draw_rect(Rect2(Vector2(bar_x, bar_start_y + 2), Vector2(STAT_BAR_WIDTH * fill_ratio, STAT_BAR_HEIGHT)), stat_colors[i].darkened(0.2))

	# 总和提示
	var total := int(stats.get("dmg", 0)) + int(stats.get("spd", 0)) + int(stats.get("dur", 0)) + int(stats.get("size", 0))
	draw_string(font, Vector2(start_x + panel_width - 50, bar_start_y + 20), "SUM: %d" % total, HORIZONTAL_ALIGNMENT_RIGHT, -1, 8, Color(0.4, 0.35, 0.5, 0.6))

	# 和弦模式下显示已选音符
	if _edit_mode == "chord" and not _chord_notes.is_empty():
		var chord_text := "和弦构建: "
		for cn in _chord_notes:
			chord_text += MusicData.WHITE_KEY_STATS.get(cn, {}).get("name", "?") + " "
		chord_text += "(%d/3)" % _chord_notes.size()
		draw_string(font, Vector2(start_x + 8, start_y + INFO_PANEL_HEIGHT - 6), chord_text, HORIZONTAL_ALIGNMENT_LEFT, -1, 9, Color(1.0, 0.84, 0.0, 0.8))

# ============================================================
# 预设模板面板（新增）
# ============================================================

func _draw_template_panel(start_x: float, start_y: float, font: Font) -> void:
	var panel_width := custom_minimum_size.x - 20

	# 标签
	draw_string(font, Vector2(start_x + 2, start_y + 10), "PRESETS:", HORIZONTAL_ALIGNMENT_LEFT, -1, 8, Color(0.4, 0.35, 0.5))

	# 模板按钮
	var btn_start_x := start_x + 60
	var btn_width := 55.0
	var btn_height := 22.0

	for i in range(PRESET_TEMPLATES.size()):
		var template: Dictionary = PRESET_TEMPLATES[i]
		var btn_rect := Rect2(Vector2(btn_start_x + i * (btn_width + 4), start_y - 2), Vector2(btn_width, btn_height))

		var btn_color: Color = template["color"]
		btn_color.a = 0.5 if i == _hover_template_idx else 0.2
		draw_rect(btn_rect, btn_color)
		...
		# 模板名称
		draw_string(font, btn_rect.position + Vector2(4, 14), template["name"], HORIZONTAL_ALIGNMENT_LEFT, -1, 8, Color.WHITE)

	# 悬停时显示模板描述
	if _hover_template_idx >= 0 and _hover_template_idx < PRESET_TEMPLATES.size():
		var template: Dictionary = PRESET_TEMPLATES[_hover_template_idx]
		draw_string(font, Vector2(start_x + 60, start_y + 28), template["desc"], HORIZONTAL_ALIGNMENT_LEFT, -1, 8, Color(0.6, 0.55, 0.7, 0.8))
...
func _draw_context_menu(font: Font) -> void:
	...
	if menu_pos.y + menu_height > size.y:
		menu_pos.y = size.y - menu_height - 5
...
func _handle_mouse_button(event: InputEventMouseButton) -> void:
	...
	if event.button_index == MOUSE_BUTTON_LEFT:
		...
		# 右键：检查是否在小节区域内，显示上下文菜单
		var cell_idx := _get_cell_at_position(pos)
		if cell_idx >= 0:
			_context_menu_measure = cell_idx / BEATS_PER_MEASURE
			_context_menu_position = pos
...
func _handle_mode_button_click(pos: Vector2) -> bool:
	...
	var palette_start_x := start_x + 42
	var btn_x := palette_start_x + 7 * (PALETTE_CELL_SIZE.x + PALETTE_MARGIN) + 10
	var btn_y := 20.0 + PALETTE_Y_OFFSET
...
func _update_tooltip(pos: Vector2) -> void:
	...
	if cell_idx >= 0 and cell_idx < _sequencer_data.size():
		...
		match slot_type:
			...
			"chord":
				_tooltip_text = "和弦 — 小节开始时触发和弦法术\n占据整个小节(4拍)\n小节%d  [右键:小节操作]" % (measure_idx + 1)
				_tooltip_visible = true
			"rest":
				_tooltip_text = "休止符 — 蓄力加成\n连续2个休止符触发留白清洗\n小节%d 第%d拍  [右键:小节操作]" % [measure_idx + 1, beat_idx + 1]
				_tooltip_visible = true
...
func _get_template_at_position(mouse_pos: Vector2) -> int:
	...
	var start_x := (size.x - total_width) / 2.0 + 10.0
	var btn_start_x := start_x + 60
	var btn_y := 20.0 + TEMPLATE_Y_OFFSET - 2
	var btn_width := 55.0
	var btn_height := 22.0

	if mouse_pos.y < btn_y or mouse_pos.y > btn_y + btn_height:
		return -1

	for i in range(PRESET_TEMPLATES.size()):
		var btn_rect := Rect2(Vector2(btn_start_x + i * (btn_width + 4), btn_y), Vector2(btn_width, btn_height))
		if btn_rect.has_point(mouse_pos):
			return i
...
func _get_context_menu_item(mouse_pos: Vector2) -> int:
	...
	if menu_pos.x + menu_width > size.x:
		menu_pos.x = size.x - menu_width - 5
	if menu_pos.y + items_count * item_height + 4 > size.y:
		menu_pos.y = size.y - items_count * item_height - 9
